...
select count(codigo_cliente) from cliente where nombre_contacto = 'Maria';
...
select replace(linea_direccion1, 'C/', 'CALLE ') from cliente;
...
select empleado.nombre, count(cliente.codigo_empleado_rep_ventas) from empleado join cliente on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas group by empleado.nombre;
--9. ¿Cuántos códigos postales de clientes tienen un 8 o un 9 entre sus cifras?
select codigo_postal from cliente where codigo_postal like '%8%';
...
select nombre_cliente from cliente where linea_direccion2 is null and region = 'Madrid' and linea_direccion1 like '%4';