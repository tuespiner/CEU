...
select futbolistas.nombre from futbolistas join partidos on futbolistas.id_equipo = partidos.id_equipo_casa join equipos on futbolistas.id_equipo = equipos.id 
where to_char(partidos.fecha,'dd/mm/yyyy') = to_date('01/01/2020','dd/mm/yyyy') and futbolistas.fecha_nacimiento = (select min(fecha_nacimiento) from futbolistas;);
